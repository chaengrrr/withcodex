...
const port = Number(process.env.NOTION_LOCAL_API_PORT ?? 8787);
...
function requireConfig(res) {
  if (!process.env.NOTION_API_KEY) {
    res.status(500).json({ error: "NOTION_API_KEY is missing" });
    return false;
  }
...
app.get("/api/health", (_req, res) => {
  res.json({
    ok: true,
    notionConfigured: Boolean(process.env.NOTION_API_KEY && dataSourceId),
  });
});

app.get("/api/notion/work-logs", async (_req, res) => {
  ...
  try {
    if (log.notionPageId) {
      try {
        ...
        res.json({ notionPageId: page.id, mode: "updated" });
        return;
      } catch (error) {
    ...
    res.json({ notionPageId: page.id, mode: "created" });
  } catch (error) {
    res.status(500).json({
      error: error instanceof Error ? error.message : "Unknown Notion API error",
    });
...
app.listen(port, () => {
  console.log(`Notion local API: http://127.0.0.1:${port}`);
});
